...
def cnn_model_fn_6(features, labels, mode):
    ...
    dropout1 = tf.layers.dropout(inputs=conv1, rate=0.2, training=mode == tf.estimator.ModeKeys.TRAIN)  # dropout layer
    # Pooling Layer #1
    ...
    dropout2 = tf.layers.dropout(inputs=conv2, rate=0.1, training=mode == tf.estimator.ModeKeys.TRAIN)  # dropout layer
...
def cnn_model_fn_7(features, labels, mode):
    ...
    dropout1 = tf.layers.dropout(inputs=conv1, rate=0.05, training=mode == tf.estimator.ModeKeys.TRAIN)  # dropout layer
    # Pooling Layer #1
    pool1 = tf.layers.max_pooling3d(inputs=dropout1, pool_size=[4, 4, 4], strides=4)
    ...
    dropout2 = tf.layers.dropout(inputs=conv2, rate=0.2, training=mode == tf.estimator.ModeKeys.TRAIN)  # dropout layer
    pool2 = tf.layers.max_pooling3d(inputs=dropout2, pool_size=[4, 4, 4], strides=4)

    # Dense Layer
    pool2_flat = tf.reshape(pool2, [-1, 4 * 4 * 2 * 8])
    dense = tf.layers.dense(inputs=pool2_flat, units=16, activation=tf.nn.relu)
    dropout = tf.layers.dropout(inputs=dense, rate=0.35, training=mode == tf.estimator.ModeKeys.TRAIN)
...
def cnn_model_fn_8(features, labels, mode):
    """Model function for CNN."""
    # Input Layer
    with tf.Session() as sess:
        shape = tf.Tensor.eval(features["shape"], session=sess)
...
def main(unused_argv):
    ...
    model = (cnn_model_fn_8, "./outmodel8_expand_processed")
